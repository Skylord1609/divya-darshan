...
export const App = () => {
  ...
  // Disable browser's automatic scroll restoration for consistent behavior.
  useEffect(() => {
  ...
  const renderView = () => {
// FIX: Memoize yatraPlanProps to prevent unnecessary re-renders of child components.
    const yatraPlanProps = useMemo(() => ({ yatraPlan: yatraPlan.map(item => item.temple), isInYatraPlan, onToggleYatraPlan: toggleInYatraPlan }), [yatraPlan, isInYatraPlan, toggleInYatraPlan]);
    switch(view) {
      case 'home':
